...
const CustomTooltip = ({
  ...
  if (active && payload && payload.length) {
    return <div className="bg-[#1a1a1a] p-4 rounded-lg border border-gray-800">
        {payload.map((entry, index) => <p key={index} style={{
        color: entry.color
      }}>
            {`${entry.dataKey === 'sales' ? 'Sales' : entry.dataKey === 'rentals' ? 'Rentals' : entry.dataKey === 'price' ? 'Price: $' : 'Mindshare: '}${entry.dataKey === 'mindshare' ? (entry.value * 100).toFixed(2) + '%' : entry.value.toFixed(entry.dataKey === 'price' ? 4 : 0)}`}
          </p>)}
      </div>;
  }
  return null;
...
export default function ChartDemo() {
  ...
  return <div className="w-full max-w-6xl mx-auto p-6 bg-[#121212] rounded-xl">
      <div className="grid grid-cols-4 gap-4 mb-8 text-white">
        <div className="bg-[#1a1a1a] p-4 rounded-lg border border-gray-800">
          <div className="flex items-baseline gap-2">
            <h3 className="text-sm text-gray-400">Market cap</h3>
            <span className="text-xs text-gray-600 my-0 py-0 px-0 mx-[45px]">#633</span>
          </div>
      ...
      <div className="h-[400px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data}>
            ...
            <YAxis yAxisId="left" stroke="#666" tick={{
            fill: '#666'
          }} domain={[0, (dataMax: number) => dataMax * 1.2]} />
            <YAxis yAxisId="right" orientation="right" stroke="#666" tick={{
            fill: '#666'
          }} domain={[0, 0.5]} />
            <Tooltip content={<CustomTooltip />} />
            <Bar dataKey="sales" fill="url(#colorSales)" yAxisId="left" barSize={20} strokeWidth={2} stroke="#11f7b1" />
            <Bar dataKey="rentals" fill="url(#colorRentals)" yAxisId="left" barSize={20} strokeWidth={2} stroke="#845EF7" />
            <Line type="monotone" dataKey="price" stroke="#11f7b1" strokeWidth={2} dot={{
            fill: '#11f7b1',
            r: 4
          }} yAxisId="left" onMouseEnter={data => {
            if (data && typeof data === 'number') {
              setHoveredPrice(data);
            }
          }} onMouseLeave={() => setHoveredPrice(null)} />
            <Line type="monotone" dataKey="mindshare" stroke="#666" strokeWidth={2} dot={{
            fill: '#666',
            r: 4
          }} yAxisId="right" />
          </ComposedChart>
